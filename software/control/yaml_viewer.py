...
class YamlDataManager:
    ...
    def _create_default_templates(self):
        ...
        default_templates = {
            ...
            "incubate": {"description": "Incubate (pause)", "type": "timer", "duration": 10},
        }
...
class YamlTreeModel(QAbstractItemModel):
    def __init__(self, data, parent=None):
        ...
        if isinstance(data, dict) and "experiment" in data:
            ...
            print(
                f"Warning: No experiment data found in YAML document. Contents: {list(data.keys() if isinstance(data, dict) else [])}"
            )
                        ...
                        for i, sequence in enumerate(round_value):
                            ...
                            if isinstance(sequence, dict) and len(sequence) == 1:
                                for seq_key, seq_value in sequence.items():
                                    ...
                                    is_imaging = False
                                    if (
                                        isinstance(seq_value, dict)
                                        and "type" in seq_value
                                        and seq_value["type"] == "imaging"
                                    ):
                                        is_imaging = True

                                    seq_data = {"key": seq_key, "value": seq_value}
                                    seq_item = YamlTreeItem(
                                        seq_data, round_item, "imaging" if is_imaging else "sequence"
                                    )
...
class EditSequenceDialog(QDialog):
    ...
    def setupPropertyEditors(self, sequence_data):
        ...
        sorted_props = sorted(
            sequence_data,
            key=lambda x: (0 if x[0] == "description" else 1 if x[0] == "type" else 2 if x[0] == "operation" else 3),
        )
            ...
            if isinstance(actual_value, dict):
                ...
                for i, item in enumerate(actual_value):
                    ...
                    delete_btn.clicked.connect(
                        lambda checked, w=item_widget, e=item_edit: self.removeListItem(key, w, e, list_editors)
                    )
                ...
                if key == "channels":
                    ...
                    add_button.clicked.connect(
                        lambda: self.addChannelItem(key, list_layout, list_editors, item_layouts)
                    )
    ...
    def addChannelItem(self, key, list_layout, list_editors, item_layouts):
        ...
        delete_btn.clicked.connect(
            lambda checked, w=item_widget, e=new_item_edit: self.removeListItem(key, w, e, list_editors)
        )
    ...
    def get_sequence_values(self):
        ...
        for key, editor in self.prop_entries.items():
            if isinstance(editor, QLineEdit):
                ...
                # List value
                result[key] = [
                    self.convertValue(item_edit.text().strip()) for item_edit in editor if item_edit.text().strip()
                ]
...
class YamlViewer(QWidget):
    ...
    def __init__(self, yaml_manager, parent=None):
        ...
        # Help text
        help_label = QLabel(
            "Right-click on rounds to add sequences or remove. Right-click on sequences to edit or remove."
        )
    ...
    def add_sequence(self, index):
        ...
        if templates:
            ...
            QMessageBox.information(self, "No Templates", "No template sequences found in the YAML file.")
